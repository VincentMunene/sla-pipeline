...
SCORED_PATH = Path("data/processed/scored_transactions_demo.csv")
...
with tab1:
    ...
    if "prediction" in filtered_df.columns:
        pred_counts = filtered_df["prediction"].value_counts().rename_axis("prediction").reset_index(name="count")
        pred_counts["prediction_label"] = pred_counts["prediction"].map({0: "Not likely to breach", 1: "Likely SLA breach"})
        fig = px.bar(pred_counts, x="prediction_label", y="count", title="Predicted Breach vs Non-Breach",
                     color="prediction_label",
                     color_discrete_map={"Likely SLA breach": "#E45756", "Not likely to breach": "#2E86DE"})
        st.plotly_chart(style_plotly(fig), use_container_width=True)

    if "breach_probability" in filtered_df.columns:
        fig = px.histogram(filtered_df, x="breach_probability", nbins=30, title="Breach Probability Histogram")
...
with tab2:
    ...
    if not month_df.empty:
        fig = px.line(month_df, x="year_month", y="sla_compliance_rate", title="SLA Compliance by Month", markers=True)
    ...
    if not operator_df.empty:
        fig = px.bar(operator_df.head(15), x="primary_operator", y="sla_compliance_rate", title="Top Operators by SLA Compliance")
    ...
    if not service_df.empty:
        fig = px.bar(service_df.head(15), x="Service_Name", y="avg_breach_probability", title="Average Breach Probability by Service")
    ...
    if not bank_df.empty:
        fig = px.bar(bank_df.head(15), x="Receiver_Bank", y="avg_breach_probability", title="Average Breach Probability by Receiver Bank")
...
with tab3:
    ...
    if not loop_df.empty:
        fig = px.bar(loop_df, x="loop_count", y="transaction_count", title="Loop Count Distribution")
        fig.update_traces(marker_color="#F39C12")
        st.plotly_chart(style_plotly(fig), use_container_width=True)

    if "had_modification" in filtered_df.columns:
        mod_counts = filtered_df["had_modification"].value_counts().rename_axis("had_modification").reset_index(name="count")
        mod_counts["label"] = mod_counts["had_modification"].map({0: "No Modification", 1: "Modified"})
        fig = px.bar(mod_counts, x="label", y="count", title="Transactions Requiring Modification",
                     color="label",
                     color_discrete_map={"Modified": "#E67E22", "No Modification": "#3498DB"})
        st.plotly_chart(style_plotly(fig), use_container_width=True)

    if "n_total_operator_touchpoints" in filtered_df.columns:
        fig = px.histogram(filtered_df, x="n_total_operator_touchpoints", nbins=20, title="Operator Touchpoint Distribution")
        fig.update_traces(marker_color="#7D3C98")
        st.plotly_chart(style_plotly(fig), use_container_width=True)

with tab4:
    st.markdown('<div class="section-title">Predictive Monitoring</div>', unsafe_allow_html=True)
    high_risk_df = filtered_df.sort_values("breach_probability", ascending=False).head(50) if "breach_probability" in filtered_df.columns else filtered_df.head(50)

    st.markdown("#### High-Risk Transactions")
    display_cols = [c for c in ["Message_Type", "Sender_Bank", "Receiver_Bank", "Service_Name", "Currency", "sla_breached", "prediction", "breach_probability", "interpretation"] if c in high_risk_df.columns]
    ...
    if "start_hour" in filtered_df.columns and "prediction" in filtered_df.columns:
        hour_risk = filtered_df.groupby("start_hour")["prediction"].mean().reset_index(name="predicted_breach_rate")
        fig = px.line(hour_risk, x="start_hour", y="predicted_breach_rate", title="Predicted Breach Rate by Start Hour", markers=True)
...
with tab5:
    ...
    st.info("Forecast uses Holt-Winters Exponential Smoothing with additive trend and no seasonality.")
