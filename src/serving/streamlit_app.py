...
# # -------------------------
# st.set_page_config(
#     page_title="SLA Operations Intelligence Dashboard",
#     layout="wide"
# )
...
# # Title
# # -------------------------
# st.markdown('<div class="dashboard-title">SLA Operations Intelligence Dashboard</div>', unsafe_allow_html=True)
# st.markdown(
#     '<div class="dashboard-subtitle">Operational reporting, breach prediction, and workforce planning from payment logs.</div>',
#     unsafe_allow_html=True
# )
...
# # Standardize display categories
# for col in ["Message_Type", "Sender_Bank", "Receiver_Bank", "Service_Name", "Currency"]:
#     if col in df.columns:
#         df[col] = df[col].fillna("UNKNOWN").astype(str).str.strip()
#         df[col] = df[col].replace({"": "UNKNOWN", "UNK": "UNKNOWN", "nan": "UNKNOWN"})
...
#     c1, c2, c3, c4 = st.columns(4)
#     with c1:
#         render_kpi_card("Total Transactions", f"{kpis['total_transactions']:,}")
#     with c2:
#         render_kpi_card("Actual Breaches", f"{kpis['actual_breaches']:,}" if kpis["actual_breaches"] is not None else "N/A")
#     with c3:
#         render_kpi_card("Predicted Breaches", f"{kpis['predicted_breaches']:,}" if kpis["predicted_breaches"] is not None else "N/A")
#     with c4:
#         render_kpi_card("Avg Breach Probability", f"{kpis['avg_breach_probability']:.2%}" if kpis["avg_breach_probability"] is not None else "N/A")

#     c5, c6, c7 = st.columns(3)
#     with c5:
#         render_kpi_card("Avg Processing Minutes", f"{kpis['avg_processing_minutes']:.2f}" if kpis["avg_processing_minutes"] is not None else "N/A")
#     with c6:
#         render_kpi_card("Modification Rate", f"{kpis['modification_rate']:.2%}" if kpis["modification_rate"] is not None else "N/A")
#     with c7:
#         render_kpi_card("SLA Compliance Rate", f"{kpis['sla_compliance_rate']:.2%}" if kpis["sla_compliance_rate"] is not None else "N/A")
...
#     st.info(
#         "Forecast uses Holt-Winters Exponential Smoothing with additive trend and no seasonality. "
#         "If the historical pattern is smooth, the forecast may appear close to linear."
#     )
...
#         <div class="hero-subtitle">Operational reporting, breach monitoring, process analysis, and transaction volume forecasting from payment logs.</div>
#         <div class="hero-meta">
#             <span class="pill pill-blue">Internal Use Only</span>
#             <span class="pill pill-green">Data Source: scored_transactions.csv</span>
#             <span class="pill pill-red">Last viewed: {last_refreshed}</span>
#         </div>
#     </div>
...
#                 x="breach_probability",
#                 nbins=30,
#                 title="Breach Probability Distribution"
#             )
#             fig.update_traces(marker_color="#7D3C98")
#             fig = style_plotly(fig)
#             st.plotly_chart(fig, use_container_width=True)
...
# # -------------------------
# st.markdown(
#     '<div class="footer-note">Internal deployment only. This dashboard is designed for institution-specific payment operations data and should be hosted within the local or intranet environment.</div>',
#     unsafe_allow_html=True
# )